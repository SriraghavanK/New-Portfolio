...
function initTypingAnimation() {
  ...
  const roles = ["WEB DEVELOPER", "TECH ENTHUSIAST", "UI/UX DESIGNER", "PROBLEM SOLVER", "GAMER"]
  ...
  function setupScene() {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      if (startCube !== endCube) {
        const lineGeometry = new THREE.BufferGeometry().setFromPoints([startCube.position, endCube.position])
...
function initAboutTabs() {
  ...
  aboutTabs.forEach((tab) => {
    tab.addEventListener("click", function (e) {
      ...
      const tabContents = document.querySelectorAll("#aboutTabsContent .tab-pane")
...
function initSkillsRadarChart() {
  ...
  const chart = new Chart(ctx, {
    type: "radar",
    data: {
      labels: ["Frontend", "Backend", "Languages", "Databases", "Tools", "UI/UX"],
      datasets: [
        {
          label: "Skill Level",
